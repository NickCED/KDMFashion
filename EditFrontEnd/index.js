...
gsap.set([home, clump, ensemble, accessories, help, info], {alpha: 0, zIndex: 0});
gsap.set(nav, {alpha: 0});


attract.addEventListener('click', () => {
    let tl = gsap.timeline();
    tl.fromTo(attract,{zIndex:10, alpha:1} ,{duration: 1, alpha: 0,zIndex:0, ease: 'sine.inOut'});
    tl.fromTo(home,{zIndex: 0, alpha: 0}, {duration: 1, zIndex:10, alpha: 1, ease: 'sine.inOut', onComplete: ()=>{
        attract.classList.add('off');
        
        
    }}, '-=1');
    tl.fromTo(nav,{alpha: 0}, {duration: 1, alpha: 1, ease: 'power2.inOut'}, '-=1');
...
function addVisFeedback(){
    ...
    function createVisFeedback(e){
        ...
        gsap.fromTo(visFeedback, {scale:1,alpha:.2},{duration: 0.5, scale: 2, alpha: 0, ease: 'sine.inOut', onComplete: ()=>{
            visFeedback.remove();
        }});
    }
}
...
navLogo.addEventListener('click', () => {
    console.log('click')
    let tl = gsap.timeline();
    tl.fromTo(home,{zIndex: 10, alpha: 1}, {duration: 1, zIndex:0, alpha: 0, ease: 'sine.inOut'});
    tl.fromTo(nav,{alpha: 1}, {duration: 1, alpha: 0, ease: 'power2.inOut'}, '-=1');
    tl.fromTo(attract,{zIndex: 0, alpha: 0}, {duration: 1, zIndex:10, alpha: 1, ease: 'sine.inOut', onComplete: ()=>{
...
function homeToClump(clump, e){
    ...
    tl.fromTo(home,{zIndex: 10, alpha: 1}, {duration: 1, zIndex:0, alpha: 0, ease: 'sine.inOut'});
    tl.fromTo(clump,{zIndex: 0, alpha: 0}, {duration: 1, zIndex:10, alpha: 1, ease: 'sine.inOut'}, '-=1');
    tl.fromTo(homeBtn,{alpha: 1}, {duration: 1, alpha: 0, ease: 'power2.inOut'}, '-=1');
